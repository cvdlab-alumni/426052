...
var c_h1 = [0.2,0.008,0.102,1];
var c_h2 = [0.998,0.2686,0.4137,1];

function cuboid(){
	var x1 = 0.1+0.12*Math.random()
	var x2 = 0.08+0.08*Math.random()
	var x3 = 0.3+0.1*Math.random()
	return COLOR(c_h1)(CUBOID([x1,x2,x3]))
}


function village(n_x,dx,n_y,dy){
	o = "";
	for(i=1; i<n_x; i++){
		for(j=1; j<n_y; j++){
		o = STRUCT([T([0,1])([i*dx,j*dy])(STRUCT([COLOR(c_h2)(CUBOID([0.38,0.22,0.2])),T([0,1])([0.1*Math.random(),0.05*Math.random()])(cuboid())])),o]);	
	}}
	return o;
} 


var village0 = T([0,1,2])([4.5,0.42,1])(village(5,0.7,4,0.5));
var village1 = T([0,1,2])([2.1,3.2,1])(village(4,0.7,7,0.5));

var villages = STRUCT([village0,village1]);


////// EXERCISE 5 //////////////////////////////////////////////////////////////////////////////////////////////

var s0 = SIMPLEX_GRID([[0.12,-0.58,0.12,-0.58,0.12,-0.58,0.12,-0.58,0.12],[1.58],[0.01]]);
var s1 = SIMPLEX_GRID([[2.92],[0.08,-0.42,0.08,-0.42,0.08,-0.42,0.08],[0.01]]);

var street1 = T([0,1,2])([5,0.74,1])(STRUCT([s0,s1]));

var s2 = SIMPLEX_GRID([[0.12,-0.58,0.12,-0.58,0.12,-0.58,0.12],[3.08],[0.01]]);
var s3 = SIMPLEX_GRID([[2.22],[0.08,-0.42,0.08,-0.42,0.08,-0.42,0.08,-0.42,0.08,-0.42,0.08,-0.42,0.08],[0.01]]);

var street2 = T([0,1,2])([2.6,3.52,1])(STRUCT([s2,s3]));

var streets = STRUCT([street1,street2]);

var model = STRUCT([mountains,lakes,forests,villages,streets]);
